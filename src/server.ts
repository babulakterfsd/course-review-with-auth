...
process.on('unhandledRejection', () => {
  console.log(`unahandledRejection is detected , course review server is shutting down ...`);
  if (server) {
    server.close(() => {
      process.exit(1);
    });
...
process.on('uncaughtException', () => {
  console.log(`uncaughtException is detected , course review server is shutting down ...`);
  process.exit(1);
});
